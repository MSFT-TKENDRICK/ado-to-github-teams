...
describe('auth documentation acceptance', () => {
  it('keeps user, operator, security, and contributor guidance aligned', async () => {
    ...
    expect(testing).toContain('10/10 personas')
    expect(testing).toContain('12,528/12,528 schema-valid trace')
    expect(testing).toMatch(/Command discoverability moved from the prior P95 71\.8/)
    expect(testing).toMatch(/Error\s+prevention moved from prior P95 78\.1/)
