...
describe('auth documentation acceptance', () => {
  it('keeps user, operator, security, and contributor guidance aligned', async () => {
    ...
    expect(testing).toContain('31/31 flags')
    expect(testing).toContain('12/12 conflicts')
    expect(testing).toContain('8,736/8,736 schema-valid trace')
    expect(testing).toMatch(/Flag ergonomics moved from P95 43\.3/)
    expect(testing).toMatch(/Scope repetition remains modeled at P95 38\.6/)
    expect(testing).toMatch(/deferred\s+rather than implemented speculatively/)
