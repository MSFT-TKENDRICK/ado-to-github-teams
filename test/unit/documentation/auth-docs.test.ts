...
describe('auth documentation acceptance', () => {
  it('keeps user, operator, security, and contributor guidance aligned', async () => {
    ...
    expect(testing).toContain('10/10 conflicts')
    expect(testing).toContain('8,624/8,624 valid trace')
    expect(testing).toMatch(/immutable\s+corrected lower-layer evidence/)
    expect(testing).toContain('P95 26.4 with 0/8')
    expect(testing).toMatch(/ranking\s+correctly/)
