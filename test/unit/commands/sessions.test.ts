...
describe('sessions command rendering', () => {
  it('filters and identifies blocked sessions and their elicitations', () => {
    ...
    expect(rows[0]?.currentStage).toBe('Assigning team members')
    expect(rows[0]?.nextEvent).toContain('Resolve the blocking decision')
    const guided = renderSessionInbox(rows, 'guided')
    const compact = renderSessionInbox(rows, 'compact')

    expect(guided).toContain('Current stage: Assigning team members')
    expect(guided).toContain('Next event: Resolve the blocking decision')
    expect(compact).toContain('CURRENT STAGE')
    expect(compact).toContain('elicit-1:healing')
  })
})
