...
export function createTaskToken(secret: string, runId: string): string {
  ...
  return createHmac('sha256', secret).update(`migration-task:${runId}`).digest('base64url')
}

export function verifyTaskToken(secret: string, runId: string, suppliedToken: string): boolean {
